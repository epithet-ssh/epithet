...
`), 0600))
	...
	for _, tc := range []struct {
		...
		_, status, err := caclient.DoInventory(t.Context(), server.Client(), server.URL, token, inventoryapi.ControlRequest{Action: "list"})
	...
	require.NoError(t, err)
	response, status, err := caclient.DoInventory(t.Context(), server.Client(), server.URL, "", inventoryapi.ControlRequest{Action: "enroll", Host: &p, Credential: key})
	require.NoError(t, err)
	require.Equal(t, 202, status)
	require.Equal(t, "pending", response.Host.Status)
	require.Empty(t, response.Host.CredentialHash)
	token := idp.MintIDTokenWithClaims("unrelated-subject", time.Now().Add(time.Hour), map[string]any{"oid": "directory-admin"})
	response, status, err = caclient.DoInventory(t.Context(), server.Client(), server.URL, token, inventoryapi.ControlRequest{Action: "approve", ID: response.Host.ID, Revision: response.Host.Revision})
	require.NoError(t, err)
