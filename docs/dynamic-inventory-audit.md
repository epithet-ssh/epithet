...
| Principal-domain changes on hosts | Enrollment accepts `--principal-domain` and domain edits before local installation. Generated domains stay per-host; declared named domains can be shared with the same authorization attributes as static members. Scheduled host adoption of domain changes made during approval or later admin edits remains tracked in `508fj5h3`. |
